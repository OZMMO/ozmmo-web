...
CREATE TABLE sat.tbl_localidades (
    id SERIAL PRIMARY KEY,
    clave_localidad VARCHAR(10),
    estado_id INTEGER NOT NULL REFERENCES sat.tbl_estados(id),
...
CREATE TABLE sat.tbl_codigos_postales (
    ...
    estado_id INTEGER REFERENCES sat.tbl_estados(id),
    localidad_id INTEGER REFERENCES sat.tbl_localidades(id),
    esta_activo BOOLEAN DEFAULT true,
    created_at TIMESTAMPTZ DEFAULT NOW(),
    updated_at TIMESTAMPTZ DEFAULT NOW()
);
...
CREATE INDEX idx_localidades_estado ON sat.tbl_localidades(estado_id);
CREATE INDEX idx_colonias_cp ON sat.tbl_colonias(codigo_postal_id);
CREATE INDEX idx_cp_codigo ON sat.tbl_codigos_postales(codigo_postal);
